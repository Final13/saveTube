...
export async function createRedirectPayment(
  ...
  return ykRequest<YookassaPayment>(
    ...
    "/payments",
    {
      ...
      // email в metadata — внешняя копия привязки платёж↔плательщик (читается через API всегда,
      // в отличие от чека); урок инцидента 10.09 (docs/incident-2026-09-10.md)
      metadata: { payment_id: String(input.paymentId), email: input.email },
...
export async function createRecurrentCharge(input: {
  ...
  return ykRequest<YookassaPayment>(
    ...
    "/payments",
    {
      ...
      description: input.title,
      metadata: { payment_id: String(input.paymentId), renewal: "true", email: input.email },
      receipt: buildReceipt(input.email, input.title, input.amountRub),
    },
