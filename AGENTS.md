...
`components/rsy-banner.tsx`: блоки `R-A-{NEXT_PUBLIC_RSY_ID}-{4..8}`, ротация 30с, крестик после 5с, скрыт при ширине <830px. id партнёра 14782353. **Скрытие баннера — только `invisible` (visibility), не `hidden` (display:none)** — Яндекс отказывается рендерить в скрытый контейнер (warning `CONTAINER_IS_HIDDEN`, проверено боем 2026-08: баннер молча не показывался вообще). **Крестик — destroy блока + очистка контейнера**, не просто скрытие обёртки: креативы RTB ставят себе `visibility:visible` и игнорируют invisible-родителя (иначе закрывается только фон). Ротация после закрытия продолжается — следующий блок покажется заново (как в старом бэке). **Премиум без рекламы:** layout считает `hideAds` (email из сессии/cookie `user_email` → `hasActiveSubscription`, серверно, без вспышки; без MySQL — показывается) и футер не рендерит баннер вовсе.
