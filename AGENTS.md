...
- `GET /api/payment/provider` — публичный эндпоинт активного провайдера для модалки: фраза об автопродлении («Можно отключить в любой момент в личном кабинете») показывается только при `yookassa` **и явном логине по коду** (sessionEmail и сессия НЕ `auto`); незалогиненным и авто-сессиям от покупки — скрыта (решение владельца); при tbank скрыта всегда (разовые платежи). Стоит между тарифами и email-инпутом.
