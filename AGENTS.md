...
- `app/api/get-video-info` — задачная модель как в старом бэке: POST создаёт задачу (`lib/tasks.ts`, in-memory, TTL 10 мин, дедуп по videoId) и отвечает СРАЗУ 202 `pending` + task_id — **без inline-ожидания (решение владельца: вернули пинг)**; результат — poll `GET ?task_id` (1.5с, до 90с). Кеш-хит (55 мин) — `completed`+`data` сразу в POST, мгновенно. Ретраи 5× backoff (`lib/video-info-task.ts`), очередь `lib/task-queue.ts` (`VIDEO_INFO_CONCURRENCY` дефолт 4, FIFO, > `VIDEO_INFO_MAX_QUEUE` дефолт 100 → 429). HTTP-ошибки retriable (DC-бан = 404-заглушка), «контент недоступен» — нет. `app/api/get-segments` — синхронный.
...
- `GET /api/payment/provider` — публичный эндпоинт активного провайдера для модалки: при `yookassa` под кнопкой «Оплатить» показывается фраза об автопродлении («Можно отключить в любой момент в личном кабинете»), при tbank скрыта (разовые платежи — была бы ложью); при недоступности эндпоинта — скрыта.
...
- `proxy-node/server.js` — голый Node.js, ноль зависимостей: whitelist, 16 стримов/IP, CORS (`PROXY_NODE_ORIGIN`), `/health`, **троттлинг стрима `PROXY_SPEED_MBPS`** (лимит в МБ/с на один стрим, дефолт 2 — как Throttle bps=2MB в старом бэке; можно дробное, "0" — без лимита; token-bucket с АБСОЛЮТНЫМ расписанием — `nextSlot += chunkTime`, иначе опоздания таймера Windows ~15мс накапливаются и скорость проседает на ~17%; кредит 200мс, ресинхронизация после отставания >500мс). **Токена НЕТ (решение владельца): ноды открыты всем.** Логика зеркалит `app/api/proxy/route.ts` кроме авторизации — менять синхронно.
