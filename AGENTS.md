...
- Фронт `components/premium-modal.tsx`: тарифы → `GET /api/payment` → редирект на `PaymentURL`, поллинг `/api/payment/status` 5с×25. **При открытии по умолчанию выбран 30д** (`DEFAULT_RATE_INDEX` по `days===30`). Иконка «ускорения» — `components/speedo-icon.tsx` (анимированный спидометр, keyframes `.speedo-needle` в globals.css, `prefers-reduced-motion` учтён): в шапке модалки (`size-10 -mt-1 text-amber-500`) и на кнопке «Ускорить» в download-form (`size-6 text-amber-400`) — вместо молнии. Карточки тарифов: крупная цена + «на N дней», у 30д — зачёркнутая `Rate.oldPriceRub` (156₽, осознанно владельцем) и бейдж `-N%` (маркетинг, на списание не влияет); у тарифов без `oldPriceRub` строки скидки нет — цена центрируется по вертикали (`flex-col justify-center`); под тарифами — список бенефитов, под кнопкой — бейджики платёжек из `public/payments/` (скопированы из CanvasKit; sberpay.svg тяжёлый, 218 КБ; плашки как у email-инпута: белые в светлой, `dark:bg-zinc-800 dark:border-zinc-700` в тёмной), юр-строка — в самом низу. **Привязка подписки — по email сессии, если залогинен** (инпут = `receipt_email` для чека 54-ФЗ, пусто → email сессии); незалогиненные — параметр `email` = и привязка, и чек (авто-регистрация). Cookie `user_email` (365д) ставит status-эндпоинт, download-form — по ней, без неё — по сессии (`/api/auth/me`). **Перемонтирование модалки по `key` и открытие по `?success`/`?error` через setTimeout — осознанные обходы линта, не «чинить».**
...
- Рекуррент: `{prefix}recurrent_subscriptions` (`lib/recurrent-store.ts`, одна запись на email). Автосписания — `app/api/cron/billing` (cron раз в час, `Authorization: Bearer $CRON_SECRET`): только при провайдере yookassa, неудача → ретрай через сутки; **canceled с reason `*revoked*` (разрешение отозвано) → `deleteRecurrent`, ретраев нет** — метод мёртв навсегда. `success_streak` (успешных списаний подряд, колонка «Успешных подряд» в админке): +1 в `activateYookassaPayment` при `activated && metadata.renewal` (идемпотентно, повторы не накручивают), сброс при canceled в кроне. **Требует включённых повторных платежей в магазине ЮKassa (через менеджера).**
- Провайдер и email сессии приходят в модалку **SSR-пропсами** (`getPaymentProvider()`/`getSession()` в `app/page.tsx` и `app/download-link/page.tsx` → DownloadForm → PremiumModal) — fetch'ей `/api/payment/provider` и `/api/auth/me` у модалки нет, надписи не вспыхивают при открытии. Фраза об автопродлении («Можно отключить в любой момент в личном кабинете») показывается **всем** при `yookassa` (решение владельца), при tbank скрыта; стоит между бенефитами и email-инпутом. Юр-строка внизу модалки — по сессии: без неё «Регистрируясь…» (оплата = регистрация), с сессией «Оплачивая…». Эндпоинт `GET /api/payment/provider` оставлен публичным (внешние потребители).
- Админка «Оплата» (`app/admin/payments-panel.tsx` + `app/api/admin/payments`): таблицы **постранично по 10** (курсор по id, `?payments_before=`/`?recurrent_before=`). Автопродления: колонка «Дата подписки» (`created_at`) и **сортировка по всем колонкам** — клик по th, клиентская; при включении догружает все записи (`?recurrent_all=1`, до 1000; курсор несовместим с произвольным ORDER BY), пока сортировка активна — «Показать ещё» скрыта. Страница /admin шире сайта: breakout `max-w-6xl` из `max-w-4xl`. Счётчик автопродлений — `recurrentActiveStats()` (в т.ч. `renewed`: сколько активных продлились ≥1/≥2/≥3 раз по `success_streak`, в заголовке «Автопродления»; и `streaks` — полное распределение «продлений × тариф» включая 0, таблица под автопродлениями). Колонка «Способ» — `{prefix}payments.payment_method` (ЮKassa: «Visa •• 1234»/«SberPay»/«ЮMoney»; T-Bank NULL) и `recurrent_subscriptions.card_type/card_last4`. «Способы оплаты» — разбивка по всем оплаченным (`getPaymentMethodStats`: карты → «Банковская карта»; NULL → T-Bank/«ЮKassa — без данных»). Бэкфилл — «Подтянуть способы из ЮKassa» (POST `{action:"backfill-methods"}`, по `merchant_id`, ≤200 за раз, идемпотентно) и «Подтянуть серии» (`{action:"backfill-streaks"}` → `backfillRecurrentStreaks()`: пересчёт `success_streak` по оплаченным платежам с title `%(автопродление)%`, двухшаговый (агрегация SELECT → UPDATE по email-параметру, БЕЗ кросс-табличного сравнения строк — у payments от старого бэкенда иная коллация, подзапрос падает с «Illegal mix of collations»), идемпотентно). Блок «Динамика подписок» — 5 SVG-лайнчартов (свои, без библиотек): новые подписки, продления, спрос по тарифам, способы оплаты и **провайдеры T-Bank vs ЮKassa (все её способы одной линией), во всю ширину**; данные `getSubscriptionStats(days)` в `lib/payments-store.ts` (продление = title `%(автопродление)%`, способы нормализует `methodBasketLabel()` — карты → «Банковская карта»), окна 7/30/90 (`?stats_days=`), **только завершённые дни — сегодня исключён** (иначе неполный день роняет хвост линий и бейджи роста), бейдж роста — сравнение с предыдущим таким же периодом.
