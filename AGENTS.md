...
- `lib/yookassa.ts` — API v3 на fetch (Basic, `Idempotence-Key`). **Два магазина:** боевой `YOOKASSA_SHOP_ID`/`YOOKASSA_SECRET_KEY` и тестовый `YOOKASSA_TEST_*` — тестовый берётся автоматически на dev и на Vercel (VERCEL=1); прод (VPS) — всегда боевой. Первый платёж: redirect + `save_payment_method: true` + чек 54-ФЗ (vat_code 1). Контракт `/api/payment` общий: `{url, payment_id}`.
